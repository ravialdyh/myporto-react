...
const presetFunctions = {
  ...
  binary: {
    name: "Binary (Even/Odd)",
    func: { 1: 0, 2: 1, 3: 0, 4: 1, 5: 0, 6: 1 },
  },
...
export default function DiceRollExperiment() {
  ...
  const [customFunction, setCustomFunction] = useState<Record<number, number>>(
    presetFunctions.identity.func,
  );
  ...
  const handlePresetChange = (preset: keyof typeof presetFunctions) => {
  ...
  const tutorialSteps = [
    {
      target: "sample-space",
      content:
        "This is the sample space, showing all possible outcomes of rolling a die.",
    },
    {
      target: "random-variable-function",
      content:
        "This section defines how we map each outcome to a value, creating our random variable.",
    },
    {
      target: "range",
      content:
        "The range shows all possible values our random variable can take.",
    },
    {
      target: "roll-button",
      content:
        "Click this button to simulate rolling the die and see how the random variable behaves.",
    },
  ];
  ...
  return (
    <div className="w-full max-w-4xl mx-auto p-4 space-y-4">
      <Card className="border-2 border-black">
        <CardHeader>
          <CardTitle className="text-2xl font-bold">
            Dice Roll Experiment: Visualizing Random Variables
          </CardTitle>
        </CardHeader>
        <CardContent className="grid grid-cols-3 gap-4">
          {/* Sample Space */}
          <div
            className={`space-y-2 sample-space ${showTutorial && currentStep === 0 ? "ring-2 ring-black ring-offset-2" : ""}`}
          >
              ...
              {DiceFaces.map((DiceFace, index) => (
                <div
                  ...
                  className={`p-2 border-2 border-black rounded ${
                    rolledValue === index + 1 ? "bg-black text-white" : ""
                  }`}
                >
          ...
          <div
            className={`space-y-2 random-variable-function ${showTutorial && currentStep === 1 ? "ring-2 ring-black ring-offset-2" : ""}`}
          >
            <h3 className="text-lg font-semibold flex items-center">
              Random Variable Function
              <Popover>
          ...
          <div
            className={`space-y-2 range ${showTutorial && currentStep === 2 ? "ring-2 ring-black ring-offset-2" : ""}`}
          >
            <h3 className="text-lg font-semibold flex items-center">
              Range of Random Variable
              <Popover>
            ...
            <div className="space-y-1">
              {Array.from(new Set(Object.values(customFunction)))
                .sort((a, b) => a - b)
                .map((value) => (
                  <div
                    key={value}
                    className={`p-2 border-2 border-black rounded ${
                      rolledValue && customFunction[rolledValue] === value
                        ? "bg-black text-white"
                        : ""
                    }`}
                  >
      ...
      <Button
        onClick={rollDice}
        className={`w-full roll-button border-2 border-black bg-black text-white hover:bg-gray-800 ${
        ...
        <Card className="border-2 border-black">
          <CardContent className="pt-4">
            <p className="text-center mb-2">
              The outcome was {rolledValue}, so the random variable X takes the
              value {customFunction[rolledValue]}.
            </p>
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <Card className="max-w-md border-2 border-black">
            <CardContent className="p-4">
              ...
              <Button
                onClick={nextStep}
                className="mt-4 border-2 border-black bg-black text-white hover:bg-gray-800"
              >
